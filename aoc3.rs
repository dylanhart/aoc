...
fn main() {
    ...
    let a: Vec<Move> = lines.next().unwrap().split(',').map(Move::read).collect();
    let b: Vec<Move> = lines.next().unwrap().split(',').map(Move::read).collect();
    
    use std::collections::BTreeSet;
    use std::collections::HashMap;
    
    fn get_seen(moves: &[Move]) -> (BTreeSet<(i16, i16, i16)>, HashMap<(i16, i16), i32>) {
        ...
        for m in moves.iter() {
            ...
            for _ in 0..m.mag() {
                ...
                x += dx;
                y += dy;
                seen.insert((x.abs() + y.abs(), x, y));
                steps.entry((x, y))
                    // .and_modify(|v| *v = std::cmp::min(*v, step))
                    .or_insert(step);
            }
        }
        (seen, steps)
    ...
    let (apoints, asteps) = get_seen(&a[..]);
    let (bpoints, bsteps) = get_seen(&b[..]);
    
    let mut intersects = apoints.intersection(&bpoints);
    let (dist, x, y) = intersects.next().unwrap();
    
    println!("p1: {}", dist);
    
    let steps_for = |x, y| {
        *asteps.get(&(x, y)).unwrap() + *bsteps.get(&(x, y)).unwrap()
    };
    
    let mut steps = steps_for(*x, *y);
    for (_, x, y) in intersects {
        let s = steps_for(*x, *y);
        if s < steps {
            steps = s;
        }
